...
function renderCart(){
  ...
  box.innerHTML=cart.map(x=>{
    ...
    return `<div class="cartItem">
      ...
      <div>
        <b>${p.name}</b>
        <div class="qty"><button onclick="change(${p.id},-1)">−</button><span>${x.qty}</span><button onclick="change(${p.id},1)">+</button></div>
      </div>
      <b>${money(sum)}</b>
    </div>`;
  }).join("");
...
function placeOrder(){
  if(!cart.length)return alert("Please add products to cart.");
  let name=document.getElementById("name").value.trim(),
      phone=document.getElementById("phone").value.trim(),
      address=document.getElementById("address").value.trim(),
      payment=document.getElementById("payment").value;

  if(!name||!phone||!address)return alert("Please fill name, mobile and delivery address.");

  let total=cart.reduce((s,x)=>s+products.find(p=>p.id===x.id).price*x.qty,0);
  let id="SP"+Date.now().toString().slice(-8);
...
function renderOrders(){
  let b=document.getElementById("ordersBox");
  if(!orders.length){b.textContent="No orders yet.";return}
  b.innerHTML=orders.map(o=>`
    <div class="order">
      ...
      <br><strong>${money(o.total)}</strong> • ${o.payment}
    </div>`).join("");
}
